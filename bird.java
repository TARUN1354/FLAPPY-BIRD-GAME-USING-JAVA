...
public class bird extends JPanel implements ActionListener, KeyListener {
    int width = 360;
    int height = 640;
    Image bottompipe; //pipeimg
    Image toppipe; //similar to remaining 2
    Image birdimg;
    Image background;
    int birdx = width / 8; // 360/8
    int birdy = height / 2; // 640/2
    int birdwidth = 34;  //birdsize
    class flappy {   //this class represents bird character
        int x = birdx;
        int y = birdy;
        int bwidth = birdwidth;
        int bheight = birdwidth;
        public Image img; //stores image
        flappy(Image img) { //constructor
            this.img = img; //"this" is used, because constructor parameter and instance variable's name are same
        }
    }
    // Pipe
    int pipex = width;
    ...
    // Game logic
    flappy Flappy; //object represents the bird
    int velocityX = -4;  // Move pipe to left speed
    int velocityY = 0; // Move bird up and down
    int gravity = 1;
    ArrayList<pipe> pipes;  //an arraylist which is used to store pipes
    Timer gameloop;  //ensure smoothness, FPS rate
    Timer placepipeTimer; // regular intervals la pipes apprear agum as an obstacles for bird.
    boolean gameover=false; //initially game isn't over
    double score=0; //updates score
    bird() {
